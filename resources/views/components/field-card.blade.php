<article
  ...
  <div class="p-md flex flex-col sm:flex-row gap-md">
    <div class="w-full sm:w-28 h-28 bg-surface border border-outline-variant rounded-lg overflow-hidden shrink-0 flex items-center justify-center">
      @if ($cancha->imagen)
        <img class="w-full h-full object-cover" alt="Foto de la cancha {{ $cancha->nombre }}" src="{{ $cancha->imagen }}" />
      @else
        <span class="material-symbols-outlined text-outline-variant text-[36px]">terrain</span>
      @endif
    </div>
    <div class="flex-1 space-y-2 min-w-0">
      <div class="flex justify-between items-start gap-2">
        <div class="min-w-0">
          <h2 class="font-display text-lg text-on-surface uppercase truncate">
            <a href="{{ route('canchas.show', $cancha) }}" class="hover:text-primary transition-colors">{{ $cancha->nombre }}</a>
          </h2>
          <p class="text-sm text-on-surface-variant flex items-center gap-1 truncate">
            <span class="material-symbols-outlined text-[15px] shrink-0">location_on</span> {{ $cancha->direccion ?? $cancha->region }}
          </p>
        </div>
        <div class="flex flex-col items-end shrink-0">
          <span class="text-accent font-display text-lg leading-none">{{ number_format((float) $cancha->calificacion, 1) }}</span>
          <div class="flex text-accent" aria-label="{{ $cancha->calificacion }} de 5 estrellas">
      ...
      <div class="flex flex-wrap gap-1.5">
        <span
          class="px-1.5 py-0.5 bg-primary-container text-on-primary-container text-[10px] font-semibold rounded border border-primary/30">{{ $cancha->categoria }}</span>
        <span
          class="px-1.5 py-0.5 bg-surface-container-high text-on-surface-variant text-[10px] font-semibold rounded border border-outline-variant">{{ $cancha->region }}</span>
        @if ($cancha->estacionamiento)
          <span
            class="px-1.5 py-0.5 bg-accent-container text-accent text-[10px] font-semibold rounded border border-accent/30">Estacionamiento</span>
        @endif
      </div>
      @if ($cancha->descripcion)
        <p class="text-sm text-on-surface-variant line-clamp-2">{{ $cancha->descripcion }}</p>
      @endif
    </div>
  </div>
