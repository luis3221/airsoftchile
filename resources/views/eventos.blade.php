...
<header class="fixed top-0 w-full z-50 bg-surface border-b border-outline-variant">
  <nav class="flex justify-between items-center px-gutter h-16 max-w-container-max mx-auto">
    ...
    <div class="hidden md:flex items-center gap-xl">
      <a class="text-on-surface border-b-2 border-accent pb-1 text-sm font-medium" href="#" aria-current="page">Eventos</a>
      <a class="text-on-surface-variant hover:text-on-surface transition-colors text-sm font-medium" href="#">Foro</a>
    ...
    <div class="flex items-center gap-sm">
      <div class="relative hidden sm:block">
        ...
        <input id="search-mission" class="bg-surface-container-low border border-outline-variant rounded text-on-surface text-sm py-2 pl-9 pr-3 focus:border-accent focus:ring-0 w-48 font-mono placeholder:text-on-surface-variant/60" placeholder="Buscar misión…" type="text"/>
      </div>
...
<div id="mobile-menu" class="fixed inset-x-0 top-[6.5rem] z-40 md:hidden bg-surface-container border-b border-outline-variant -translate-y-[150%] opacity-0 pointer-events-none">
  <div class="flex flex-col p-gutter gap-xs">
    <a href="#" class="px-4 py-3 rounded-lg bg-primary-container text-on-primary-container font-medium">Eventos</a>
    <a href="#" class="px-4 py-3 rounded-lg text-on-surface-variant hover:bg-surface-container-high">Foro</a>
...
<aside class="hidden lg:flex flex-col fixed left-0 top-[6.5rem] h-[calc(100%-6.5rem)] z-30 bg-surface-container border-r border-outline-variant w-60">
  ...
  <nav class="flex-1 flex flex-col gap-xs p-2 pt-md">
    ...
    <a class="bg-primary-container text-on-primary-container rounded-lg px-4 py-3 flex items-center gap-md text-sm font-medium" href="#" aria-current="page">
      <span class="material-symbols-outlined text-[20px] fill-icon">event</span> Eventos
    </a>
...
<main class="lg:pl-60 pt-[6.5rem] pb-24 lg:pb-12 px-gutter grid-overlay min-h-screen">
  <div class="max-w-container-max mx-auto py-lg">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end gap-md mb-lg">
      <div>
        <h1 class="font-display text-3xl sm:text-4xl text-on-surface uppercase tracking-tight mb-xs">Calendario de despliegue</h1>
        <p class="text-on-surface-variant max-w-xl leading-relaxed">Monitorea los teatros de operaciones activos en todo el territorio. Todas las misiones requieren verificación previa y charla de seguridad.</p>
      </div>
      <span class="font-mono text-[11px] text-primary bg-primary-container px-3 py-1.5 rounded border border-primary/30 uppercase tracking-wide shrink-0">Estado: operativo</span>
    </div>

    <!-- Filter bar -->
    <section class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-sm mb-xl p-xs bg-surface-container border border-outline-variant rounded-lg" aria-label="Filtros de eventos">
      <div class="flex flex-col gap-1 p-md bg-surface-container-low rounded-lg border border-outline-variant/50">
        <label for="filter-region" class="font-mono text-[11px] text-on-surface-variant uppercase tracking-wide">Región</label>
        <select id="filter-region" class="bg-transparent border-none text-on-surface font-display text-lg focus:ring-0 cursor-pointer p-0">
      ...
      <div class="flex flex-col gap-1.5 p-md bg-surface-container-low rounded-lg border border-outline-variant/50">
        ...
        <div class="flex gap-xs" role="group" aria-label="Tipo de juego">
          <button class="px-3 py-1 bg-primary text-on-primary font-mono text-[12px] rounded" aria-pressed="true">Milsim</button>
          <button class="px-3 py-1 bg-surface-container-high text-on-surface-variant font-mono text-[12px] rounded hover:text-on-surface" aria-pressed="false">Speed</button>
          <button class="px-3 py-1 bg-surface-container-high text-on-surface-variant font-mono text-[12px] rounded hover:text-on-surface" aria-pressed="false">Rec</button>
        </div>
      </div>
      <div class="flex flex-col gap-1 p-md bg-surface-container-low rounded-lg border border-outline-variant/50">
        <label for="filter-month" class="font-mono text-[11px] text-on-surface-variant uppercase tracking-wide">Rango de fechas</label>
        <input id="filter-month" class="bg-transparent border-none text-on-surface font-display text-lg focus:ring-0 w-full p-0" type="month" value="2026-07"/>
      </div>
      <div class="flex items-center p-md">
        <button class="flex items-center justify-center gap-2 w-full bg-surface-container-high text-on-surface font-mono text-[12px] uppercase py-3 rounded border border-outline-variant hover:bg-accent hover:text-on-accent hover:border-accent transition-colors">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-lg">
      ...
      <!-- Event 1 -->
      <article class="group bg-surface-container border border-outline-variant rounded-lg overflow-hidden flex flex-col hover:border-primary/50 transition-colors">
        <div class="relative h-44 overflow-hidden">
          <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-black/70 to-transparent p-md z-10">
            <span class="font-mono text-[11px] text-accent bg-black/40 px-2 py-1 border border-accent/40 rounded uppercase">Milsim operativo</span>
          </div>
          <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Jugadores de airsoft en equipo táctico completo avanzando por un bosque denso durante una misión nocturna simulada." src="https://lh3.googleusercontent.com/aida-public/AB6AXuCDPg6T3lP-Wd0Mq-Z9dl9IQ4TtI49PY6UoTgSDmX28SGtt9fcUjzwTkgXHrcROugANysrWqINHjVoARMyh9ruwV4MugVWKdkDkQ8l0jc8vXdrR0JtnPACxF9PK4Q-l9H4uU1qxMf4UMbN8NoMIgD6m3EDgOiXv9nqwAEbYq5kW_c61wEkUid5b80n7bDI2zdfnbdNniiErxQbBx0osTJN950UrFibJJ1etjlWvDkQ40YFImZo0w7kaCA"/>
        </div>
        <div class="p-lg flex flex-col flex-grow">
          ...
          <div class="mt-auto pt-md border-t border-outline-variant flex items-center justify-between gap-2">
            <div class="flex -space-x-2" aria-label="42 operadores inscritos">
              <div class="w-8 h-8 rounded-full bg-surface-container-high border-2 border-surface-container flex items-center justify-center text-[10px] font-bold">JD</div>
              <div class="w-8 h-8 rounded-full bg-primary border-2 border-surface-container flex items-center justify-center text-[10px] font-bold text-on-primary">AT</div>
              <div class="w-8 h-8 rounded-full bg-accent border-2 border-surface-container flex items-center justify-center text-[10px] font-bold text-on-accent">+42</div>
            </div>
      ...
      <article class="group bg-surface-container border border-outline-variant rounded-lg overflow-hidden flex flex-col hover:border-primary/50 transition-colors">
        <div class="relative h-44 overflow-hidden">
          <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-black/70 to-transparent p-md z-10">
            <span class="font-mono text-[11px] text-on-surface bg-black/40 px-2 py-1 border border-outline-variant rounded uppercase">Torneo speedsoft</span>
          </div>
          <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Competencia de speedsoft de alta intensidad en una arena techada iluminada con luces de neón." src="https://lh3.googleusercontent.com/aida-public/AB6AXuADZYQKIyLtnKNFJtPjVYfyoXTuPIgALNTGh9Fl0c0GwwPfORG2OAyoLDD1NZzS0b3OG5JdggTvU7AI04oKXlYawHRTveX1_EgIBufG88kYj9eWzo0uVyQwZQgGwGVwKbJwTSnVJPk36q7W7TwdUKUCgTK6vfH-bBF8EfoDc3aWrDJkg4M6abikmFBm7x_iDR5NX3I84Qbp-83ERuJXBAv3_xvitREePqXs-FXN2OJJu2qiVS_572FLHw"/>
        </div>
        <div class="p-lg flex flex-col flex-grow">
          <div class="flex justify-between items-start gap-2 mb-md">
            <div>
      ...
      <article class="group bg-surface-container border border-outline-variant rounded-lg overflow-hidden flex flex-col hover:border-primary/50 transition-colors">
        <div class="relative h-44 overflow-hidden">
          <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-black/70 to-transparent p-md z-10">
            <span class="font-mono text-[11px] text-primary bg-black/40 px-2 py-1 border border-primary/40 rounded uppercase">Sesión recreativa</span>
          </div>
          <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Fábrica industrial abandonada reconvertida en campo de airsoft, con estructuras oxidadas y luz de mañana." src="https://lh3.googleusercontent.com/aida-public/AB6AXuDXFhN2U-mf_RY0vNsOf10z2NMsT4FGe9sCeZsvl7xDmA8ZyK2G8AunHAL9b6zZQBtGkxUnqqJoHBKSNFvj481xUn7KhmJTn8UM8m4x1TQ3TrCmuPdDvGEA39jqW1JjijHD4LkGzbLKGCW2tShSbPEvQQlX6NR4dLuBqVF27rGsGm_JfbaJl2NWzQfLH32HnSHKnmqH5nDq4vto7hKYTKJ9khPUARmNa128gA7gE806nggNIvuN575v2w"/>
        </div>
        <div class="p-lg flex flex-col flex-grow">
          <div class="flex justify-between items-start gap-2 mb-md">
            <div>
      ...
      <article class="group bg-surface-container border border-outline-variant rounded-lg overflow-hidden flex flex-col hover:border-primary/50 transition-colors">
        <div class="relative h-44 overflow-hidden">
          <div class="absolute top-0 left-0 w-full bg-gradient-to-b from-black/70 to-transparent p-md z-10">
            <span class="font-mono text-[11px] text-accent bg-black/40 px-2 py-1 border border-accent/40 rounded uppercase">Milsim 24h</span>
          </div>
          <img class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="Operadores de airsoft con trajes ghillie ocultos entre pastizales altos, vistos a través de una mira." src="https://lh3.googleusercontent.com/aida-public/AB6AXuAK2p_0SPicIuNZtFj7ERjvyFARaMedp0lQEzG5NKAKCSkyiMh7LOiXHh7IJlMexYKuqF98aqUG0WSYQJhXdtC4PsWSmqtHL8NQFosJsEj_hD1Jo1qHFpB4qv21Fr2ZVqJGXz27i7oSH0kHBByPwCtSrUAiqf3PRCgdk7cR2Llr8BjqHln94OFr8ZnfdI3mHYt8pL2j0_ZvIrsaRCaAd8X8yJnAF8c2jTSpFTKQI_anaXB41dP5KhINTg"/>
        </div>
        <div class="p-lg flex flex-col flex-grow">
          <div class="flex justify-between items-start gap-2 mb-md">
            <div>
    ...
    <section class="mt-xl">
      <div class="bg-surface-container border border-outline-variant rounded-lg p-lg flex flex-col md:flex-row gap-lg">
        <div class="flex-grow rounded border border-outline-variant relative overflow-hidden h-72 md:h-auto">
          ...
          <img class="absolute inset-0 w-full h-full object-cover grayscale opacity-30" alt="Mapa de Chile mostrando la densidad regional de operaciones de airsoft activas." src="https://lh3.googleusercontent.com/aida-public/AB6AXuCdstGHJ0gLGySrjdB4D5jzXdbfDNvGJA3RI8DOJoz8CLRc24J1F-w_YSGS8mpM3ySdONaeL2XLTwpa65wlz9cdsU145SWlOZqNw7CyfyntVFeW6Kdurx7HAZ6lfGZOK9_8jKbmDlng071RMoCs6ZP3TosNPNhilg2n-7G3G0-wc4XfJfMkky6S8JvAYodm-JSMjsAAC6gH11HhfkkpBl70AhOHd3ZiheF9b1W3wMWU73IbgOM74wy8Aw"/>
        ...
        <div class="w-full md:w-80 flex flex-col gap-md shrink-0">
          ...
          <div class="space-y-sm">
            <div class="flex justify-between font-mono text-[12px] p-sm bg-surface-container-low border-l-2 border-primary rounded-r">
              <span class="text-on-surface-variant">Santiago (RM)</span><span class="text-primary font-semibold">12 activos</span>
            </div>
            <div class="flex justify-between font-mono text-[12px] p-sm bg-surface-container-low border-l-2 border-outline-variant rounded-r">
              <span class="text-on-surface-variant">Valparaíso (V)</span><span class="text-on-surface">5 activos</span>
            </div>
            <div class="flex justify-between font-mono text-[12px] p-sm bg-surface-container-low border-l-2 border-outline-variant rounded-r">
          ...
          <div class="mt-auto p-md bg-surface-container-low rounded border border-outline-variant">
            <p class="text-sm text-on-surface-variant leading-relaxed">Selecciona un marcador para ver los informes de misión y la topología del terreno de esa región.</p>
          </div>
        </div>
      </div>
    </section>
